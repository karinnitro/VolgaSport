...
from tkinter import ttk, messagebox # Расширенные виджеты и всплывающие окна
from PIL import Image, ImageTk # Для работы с изображениями

from buy_product import ProductPurchaseWindow # Окно покупки
from review_window import ProductReviewWindow # Окно добавления отзыва
from view_reviews import ViewProductReviewsWindow # Окно просмотра отзывов
...
def show_purchased_products():
    """Отображает окно истории покупок с таблицей всех покупок."""
...
def show_store_window(username):
    """Запускает основное окно магазина с товраами и действиями."""
    init_store_db()
    ...
    store_window.configure(bg="#D9EBFF")  
    ...
    content_frame.rowconfigure(0, weight=1)
     ...
     # Фиксированная высота строки в Treeview 
    style.configure('Treeview.Heading', anchor='center')
    tree.pack(expand=True, fill='both')
    ...
    def get_selected_product_info():
        """Возвращает данные о выбранном товаре."""
        selected = tree.focus()
        if not selected:
    ...
    def on_close():
        """Закрывает окно магазина и соединение с БД."""
        conn.close()
